...
public:
  ...
  ping_message(const ping_message& msg)
      : _bufferLength{msg.msgDataLength()}, msgData{(uint8_t*)malloc(sizeof(uint8_t) * _bufferLength)} {
    memcpy(msgData, msg.msgData, _bufferLength);
  }

  ping_message(const uint16_t bufferLength)
      : _bufferLength{bufferLength}, msgData{(uint8_t*)malloc(sizeof(uint8_t) * _bufferLength)} {
    if (bufferLength >= 2) {
      msgData[0] = 'B';
      msgData[1] = 'R';
    }
  ...
  ping_message(const uint8_t* buf, const uint16_t length)
      : _bufferLength{length}, msgData{(uint8_t*)malloc(sizeof(uint8_t) * _bufferLength)} {
...
public:
  ...
  uint16_t msgDataLength() const {
    return headerLength + payload_length() + checksumLength;
  } // size of entire message buffer (header, payload, and checksum)
  ...
  // to migrate legacy ping1d devices, a new message id should be used with the
  // same fields + padding
  uint16_t checksum() const {
    return msgData[msgDataLength() - checksumLength] + (msgData[msgDataLength() - checksumLength + 1] << 8);
  }
  void set_checksum(uint16_t checksum) {
    msgData[msgDataLength() - checksumLength]     = (uint8_t)checksum;
    msgData[msgDataLength() - checksumLength + 1] = checksum >> 8;
  }
